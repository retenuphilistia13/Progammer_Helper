...
snippet="classItem";
  ...
  snippet="mainClass";
    ...
    public void addOOPSnip(String language) {
        ...
        switch (language) {
            case "Java" -> {
                OOPSnip = new JavaSnippets(OOPLanguage, this, input, multi, livePrev);
                System.out.println("java intializing");
            }
            case "C++" -> {
                System.out.println("c++ intializing");
                OOPSnip = new C_Plus_PlusSnippets(OOPLanguage, this, input, multi, livePrev);
            }
    ...
    // Create and return the ActionListener
        ...
        return (ActionEvent event) -> {
            ...
            // Update the selected state of all menu items
            for(JMenuItem snips:snipList){
                if(snips==selectedMenuItem){
                    snips.setSelected(true);
                }else{
                    snips.setSelected(false);
                }
 ...
 public void firstSnippet() {
        switch (snippet) {
            case "getSetItem" -> {OOPSnip.gettersSetters();}
            case "mainClass" -> {OOPSnip.createMainClass();}
            case "classItem" -> {OOPSnip.createClass();}
            default -> {System.out.print("snippet default "+snippet);OOPSnip.createClass();}
        }
        addToFrame(OOPSnip);
   }
  public ActionEvent handleOOPFirstEvent(ActionEvent e) {
        switch (snippet) {
            case "getSetItem" -> {return new ActionEvent(getSetItem, e.getID(), e.getActionCommand(), e.getModifiers());}
            case "mainClass" -> {return new ActionEvent(mainClass, e.getID(), e.getActionCommand(), e.getModifiers());}
            case "classItem" -> {return new ActionEvent(classItem, e.getID(), e.getActionCommand(), e.getModifiers());}
            default -> {System.out.print("snippet default "+snippet); return new ActionEvent(classItem, e.getID(), e.getActionCommand(), e.getModifiers());}
        }
   }
